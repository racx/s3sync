...
module S3sync
   ...
   include S3Config
	...
	def S3sync.main 	
        ...
        [ '--public-read','-p', GetoptLong::NO_ARGUMENT ], # backward compatibility
			  [ '--acl-rule','-a', GetoptLong::REQUIRED_ARGUMENT ],	 # options: public-read or authenticated-read  			  
...
#{name} [options] <source> <destination>\t\tversion #{$S3SYNC_VERSION}
  ...
  --make-dirs           --no-md5             --authenticated-read -a
...
ENDUSAGE
	...
	class S3Node < Node
		...
		def updateFrom(fromNode)
          ...
          # headers['x-amz-acl'] = 'public-read' if $S3syncOptions['--public-read'] # backward compatibility
					headers['x-amz-acl'] = $S3syncOptions['--acl-rule'] if $S3syncOptions['--acl-rule']	
